...
//    }
    ...
    protected function applyFormula(Table $table)
    {
        ...
        foreach ($this->listFormula as $columnPath) {
            $columnBase = $this->tableRetriever->getItemFromDataPath($columnPath);
            ...
            foreach ($definitionFormula['columns'] as $columnFormulaPath) {
                ...
                if (is_array($columnFormulaPath)) {
                    $fromItem = $this->tableRetriever->getParentOrSubItemsFromGenericPath($columnFormulaPath['fromGroup'], $columnBase->getRow()->getGroup());
